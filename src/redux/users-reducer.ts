...
export const usersActions = {
  ...
  toggleFollowing: (isFetching: boolean, id: number) => ({
...
export const getUsers = (usersPerPage: number, currentPageNumber: number, filter: FilterType): ThunkType => {
  return async (dispatch, getState) => {
    dispatch(usersActions.toggleIsFetching(true));
    dispatch(usersActions.setFilter(filter));
    dispatch(usersActions.setCurrentPage(currentPageNumber));
    const response = await usersAPI.getUsers(usersPerPage, currentPageNumber, filter);  // TS не подсказывает response.items... ((
